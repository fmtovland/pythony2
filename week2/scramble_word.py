...
while i < len(user_input):	#program will loop till i reaches end
	while user_input[j] != ' ' or '.' or ',':	#after this user_input[j] will be the last letter of word
		j=j+1
	j=j-1

	print(user_input[i])	#print first letter of word

	k=0+j	#ensure k not directly mapped to object j
	while(k>i):	#print middle letters of word backwards
		print(user_input[k])
		k=k-1

	print(user_input[j])	#print last letter of word
